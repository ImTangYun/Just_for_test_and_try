...
int StreamSocketContext::HandleOutput()
{
    Packet* packet = NULL;
    do {
        packet = packet_queue_->Pop(1000);
        if (packet == NULL) {
            return -1;
        }
        // WLOG(INFO, "handling out put");

        // send protocool code
        int32_t protocool_code = 1;
        int32_t net_protocool_code = htonl(protocool_code);
        ...
        int32_t whole_length = sizeof(int32_t) + 
            packet->head_length() + sizeof(int32_t) + packet->data_length();
        int32_t net_whole_length = htonl(whole_length);
        Send((char*)(&net_whole_length), sizeof(net_whole_length));

        // send packet head data length
        int32_t head_length = htonl(packet->head_length());
        ...
        //send data length
        int32_t data_length = htonl(packet->data_length());
...
int StreamSocketContext::HandleInputImpl()
{
    while (true) {
        WLOG(DEBUG, "recving....");
        int ret = ::recv(fd_, recv_buffer_ + buffer_offset_,
                recv_buffer_length_ - buffer_offset_, 0);
        if (ret > 0) {
            buffer_offset_ += ret;
            WLOG(DEBUG, "buffer_offset_ :%d", buffer_offset_);
            Chunking();
        } else if (ret == 0) {
            delete this;
            break;
...
void StreamSocketContext::Chunking()
{
    if (buffer_offset_ >= 8) {
        int head = ntohl(*(int*)recv_buffer_);
        if (head != HEAD) {
            printf("broken packet");
            buffer_offset_ = 0;
            return;
        }
        int total_length = ntohl(*(int*)(recv_buffer_ + 4));
        WLOG(DEBUG, "total length: %d", total_length);
        if (total_length >= recv_buffer_length_) {
            printf("too big packet or unrecognized packet\n");
            buffer_offset_ = 0;
            return;
        }
        if (total_length > buffer_offset_ - 8) {
            printf("not enough data to form a packet\n");
            return;
        }

        int head_length = ntohl(*(int*)(recv_buffer_ + 4 + 4));
        char* head_data = new char[head_length + 1];
        memmove(head_data, recv_buffer_ + 4 + 4 + 4, head_length);
        head_data[head_length] = '\0';

        int data_length = ntohl(*(int*)(recv_buffer_ + 4 + 4 + 4 + head_length));

        WLOG(DEBUG, "total_len:%d, head_len:%d, body_len:%d", total_length, head_length, data_length);
        char* data = new char[data_length + 1];
        memmove(data, recv_buffer_ + 4 + 4 + 4 + 4 + head_length, data_length);
        data[data_length] = '\0';

        memmove(recv_buffer_, recv_buffer_ + total_length + 8,
                buffer_offset_ - total_length - 8);
        buffer_offset_ -= total_length;
        buffer_offset_ -= 8;
